...
function Page() {
  ...
  useEffect(() => {
    async function fetchAllBlogs() {
      setLoading(true);
      try {
        const response = await fetch(`/api/blog/get-allblogs`, {
          method: "GET",
          headers: {
        ...
        setBlogPosts(res?.data || []); // Ensure valid data before setting state
      } catch (error: any) {
    ...
    <div className="flex flex-col md:flex-row gap-6 md:h-[280px] bg-white shadow-lg overflow-hidden border border-gray-200 hover:shadow-2xl transition-all duration-300 rounded-lg">
      ...
      <div className="w-full md:w-[60%] h-[200px] md:h-auto">
        <Image
          width={380}
          height={200}
          src={blog.thumbnail || "/default-thumbnail.jpg"} // Use a proper fallback image
          alt={blog.title}
